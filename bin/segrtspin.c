...
const char *usage_msg =
	"Usage: (1) rtspin OPTIONS SEGMENTS PERIOD DURATION\n"
	"       (2) rtspin -S [INPUT] SEGMENTS PERIOD DURATION\n"
	"       (3) rtspin OPTIONS -C FILE:COLUMN SEGMENTS PERIOD [DURATION]\n"
	"       (4) rtspin -l [-a CYCLES]\n"
	"       (5) rtspin -B -m FOOTPRINT\n"
	"       (6) rtspin -a 0\n"
	"\n"
	"Modes: (1) run as periodic task with given segment times and PERIOD\n"
	"       (2) run as sporadic task with given segment times and PERIOD,\n"
	...
	"\n"
	"Required arguments:\n"
	"    SEGMENTS	       execution and suspension times for individual segments (in ms)\n"
	"		       C_1 S_1 C_2 S_2 ... C_n or C_1 Q_1 S_1 C_2 Q_2 S_2 ... C_n Q_n if -P is set\n"
	"    PERIOD            reservation parameters (in ms)\n"
	...
	"\n"
	"    -X PROTOCOL       access a shared resource protected by a locking protocol once per computation segment\n"
	...
	"\n"
	"Units:\n"
	"    SEGMENTS and PERIOD are expected in milliseconds.\n"
...
int main(int argc, char** argv)
{
	...
	if (per_segment_priorities || per_segment_releases) {
		...
		num_segments = 2 * groups - 1;


		if (num_segments > 32)
			usage("Maximum of 32 Segments.");
		...
		for (arg = 0; arg < groups; arg ++) {
			wcts[2 * arg] = ms2ns(want_positive_double(argv[optind +  2 * arg], "C"));
			segment_priorities[arg] = priority;
			if (arg != groups - 1)
				wcts[2 * arg + 1] = ms2ns(want_positive_double(argv[optind + 2 * arg + 1], "S"));
		}
	}
	...
	fprintf(stderr, "Parsed %d total segments, with total worst case running time %llu ms\n", num_segments, ns2ms(wcet));
	for (int i = 0; i < num_segments; i++) {
		if (per_segment_priorities && i % 2 == 0)
			fprintf(stderr, "Segment %d: WCET of %llu ms, priority %d\n", i, ns2ms(wcts[i]), segment_priorities[i / 2]);
		else
			fprintf(stderr, "Segment %d: WCET of %llu ms\n", i, ns2ms(wcts[i]));
	}


	period_ms = want_positive_double(argv[argc - 2], "PERIOD");
	wcet_ms = ns2ms(wcet);
	...
	if (argc - optind < 3 && cost_csv_file)
		...
		duration = want_positive_double(argv[argc - 1], "DURATION");
	...
	if (per_segment_priorities) {
		param.priority = segment_priorities[0];
	} else {
	...
	// Round up to get number of computation segments
	param.num_segments = (num_segments + 1) / 2;
	...
	while (1) {
		...
		for (int segment = 0; segment < num_segments; segment++) {
			...
			if (cost_csv_file) {
				...
				acet = (ns2ms(wcts[segment]) - drand48() * underrun_ms) * 0.001;
				if (acet < 0)
					acet = 0;
			}
			/* scale exec time */
			...
			if (segment % 2 == 0) {
				if (verbose)
					fprintf(stderr,
						"\tsegment %d, target exec. time: %6.2fms (%.2f%% of WCET)\n",
						segment,
						acet * 1000,
						(acet * 1000 / ns2ms(wcts[segment])) * 100);
				job(acet, start + duration, lock_od, cs_length * 0.001);
				signal_end_segment();
			} else {
				if (verbose)
					fprintf(stderr,
						"\tsegment %d, target suspension time: %6.2fms (%.2f%% of WCET)\n",
						segment,
						acet * 1000,
						(acet * 1000 / ns2ms(wcts[segment])) * 100);
				lt_sleep(ms2ns(acet * 1000));
			}
		}
